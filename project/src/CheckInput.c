...
int check_number(char* number) {
    ...
    if (strlen(number) > 4 || strlen(number) == 0) {
        return WRONG_NUM_ERR;
    }
    if (number[0] == '-') {
        return WRONG_NUM_ERR;
...
int parse_command(char cmd_line[80], char* name, char* surname, char* role) {
    int offset = 0;
    int elem_count = 0;
...
int parse_person_data(char buffer[80], char* name, char* surname, char* role, int* importance) {
    int tmp_importance;
